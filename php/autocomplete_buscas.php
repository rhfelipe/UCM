...
$query1 = 'SELECT cod_amb, ambiente FROM ';
$query2 = ' WHERE cod_amb ILIKE ';
$query3 = ' OR ambiente ILIKE ';
$query4 = ' OR sigla_amb ILIKE ';
//FILTRO POR CAMPUS
...
$campus=array('ccja','ccju','cccb','ccjb','ccju','lcem','lcmi','ocma','ofpa','pfcg','ocpa','lcma','lmae');
...
$filtro_npassa = array('Sala de Aula','Sala dos Técnicos','Sala do Servidor','Sala de Professor','Banheiro','Banheiro Masculino','Banheiro Feminino','Laboratório','Dispensa','Em Construção','Rampa','Manutenção','Secretaria','Copa','Sala de Estudos','Servidores','Sala de Servidores');
...
//AQUI SÃO ADICIONADAS AO VETOR AS PALAVRAS ENCONTRADAS QUE AINDA NÃO FORAM SALVAS
         foreach ($andar as $value) {
             $filtro = $query1.$campus[$i].'.'.$value[0].$query2.$key.$query3.$key.$query4.$key; //filtro da consulta principal
             $QUERY=pg_query($conexao,$filtro);
              $NUM_LINHAS=pg_num_rows($QUERY);
              if ($NUM_LINHAS>0){
                     for ($L=0;$L<$NUM_LINHAS;$L++){
                          $linha_filtrada=pg_fetch_row($QUERY,$L);
                          if (in_array($linha_filtrada[0],$data)==false and $linha_filtrada[0]!=null and in_array($linha_filtrada[0],$filtro_npassa)==false){
                             array_push($data,$linha_filtrada[0]);
                          }
                          if (in_array($linha_filtrada[1],$data)==false and $linha_filtrada[1]!=null and in_array($linha_filtrada[1],$filtro_npassa)==false){
